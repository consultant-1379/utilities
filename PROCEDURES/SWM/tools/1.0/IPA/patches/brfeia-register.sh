...
# if brfp is not installed, it is normal that we can't register BRF-EIA,
# then exit without causing problems (rc=0)

# if brfp is install, then fail if registeration fails

rc=0

if [ -f /opt/ericsson/brf/libexec/common/brf-participant-register ]
then
    if [ -f /opt/ericsson/brf/libexec/eia/brfeia-apr9010485/etc/Participant.cfg ]
    then
        /opt/ericsson/brf/libexec/common/brf-participant-register \
